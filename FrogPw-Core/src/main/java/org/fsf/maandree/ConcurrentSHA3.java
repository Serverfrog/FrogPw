...
public class ConcurrentSHA3 {
     ...
     *
     * @param n {@code Math.min(SHA3.message.length, rr) + msgoff} msgoff The
     * number of times to loop has run times the bitrate rr Bitrate in bytes
     ...
     * Squeeze the Keccak sponge
     *
     * @param withReturn Whether to return the hash instead of just do a quick
     * squeeze phrase and return {@code null}
